// kernel/executor.js
// The LLM-OS v3 execution loop. This is the operating system.
//
// v3 adds dataset export: every execution produces a JSONL training file
// with (system, user, assistant) turns suitable for fine-tuning a smaller
// model (e.g. Gemma 4 E4B) on ISA bytecode traces.
//
...
${this.state.size > 0 ? JSON.stringify(Object.fromEntries(this.state)) : '(empty)'}`;
  ...
  async step() {
    ...
    // Retry on empty response (up to 2 times)
    if (!rawText) {
      for (let retry = 0; retry < 2; retry++) {
        // Nudge the model
        this.messages.push({
          role: 'user',
          content: 'Emit one ISA opcode now. No prose.',
        });
        const retryResp = await this.backend.generate(this.messages, {
          maxTokens: 300,
          stop: stopSequences,
          temperature: 0.3,
        });
    ...
    switch (op.type) {
      ...
      case 'think':
        // Think-only turn: append as assistant, then nudge for an opcode
        this.messages.push({ role: 'assistant', content: rawText });
        this.messages.push({
          role: 'user',
          content: 'Good reasoning. Now emit exactly one ISA opcode (e.g. <|call|>...<|/call|> or <|halt|>status=...).',
        });
        break;
      ...
      default:
        // Unknown/prose — nudge the model to emit a proper opcode
        this.messages.push({ role: 'assistant', content: rawText });
        this.messages.push({
          role: 'user',
          content: 'Invalid output. Emit exactly ONE ISA opcode. Example: <|call|>tetris.observe {}<|/call|>',
        });
        break;
    }
  }
  ...
  // ── Dataset export ──────────────────────────────────────────────
  // Converts the execution trace into training data for fine-tuning.

  /**
   * Export the execution as a chat-format JSONL training dataset.
   * Each line is a complete conversation: system + user + alternating
   * assistant/user turns (opcode/result pairs).
   *
   * Filters out bad runs (unknown opcodes, parse errors) to keep
   * only clean traces suitable for fine-tuning.
   *
   * @returns {object[]} Array of training examples in chat format
   */
  exportDataset() {
    // Only export if the run produced at least some valid opcodes
    const validOps = this.trace.filter(t =>
      t.type !== 'unknown' && t.type !== 'error'
    );
    if (validOps.length === 0) return [];

    // Build one training example per complete execution
    // Format: OpenAI chat fine-tune format (messages array)
    const systemMsg = { role: 'system', content: this.buildSystemPrompt() };
    const userMsg = { role: 'user', content: this.task };

    // Walk messages to build assistant/user turn pairs
    const turns = [];
    for (let i = 2; i < this.messages.length; i++) {
      const msg = this.messages[i];
      // Skip messages with parse errors in them
      if (msg.content && msg.content.includes('__parse_error')) continue;
    ...
    return [{
      ...
      metadata: {
        status: this.haltStatus,
        steps: this.stepCount,
        cartridges: [...this.cartridges.keys()],
   ...
   * Each example is: system prompt + task + context (recent history) +
   * one assistant turn (the opcode). This is useful for step-level
   * fine-tuning where each opcode is an independent prediction.
   *
   * @param {number} [contextWindow=4] - Number of prior turns to include as context
   * @returns {object[]} Array of training examples
   */
  exportStepDataset(contextWindow = 4) {
    const examples = [];
    const systemMsg = { role: 'system', content: this.buildSystemPrompt() };
    const userMsg = { role: 'user', content: this.task };

    // Walk through messages finding assistant turns (opcodes)
    for (let i = 2; i < this.messages.length; i++) {
      const msg = this.messages[i];
      ...
      if (msg.content && msg.content.includes('unknown')) continue;

      // Gather context: preceding turns within the window
